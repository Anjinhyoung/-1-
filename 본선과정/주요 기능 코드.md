...
-- 로비에서 게임으로 이동하는 GUI 프레임을 가져옵니다.
local frame = player.PlayerGui['Lobby_to_Game'].Frame
-- 디바운스 변수는 버튼이 여러 번 눌리는 것을 방지합니다.
...
function teleport()
    ...
    if not debounce then
        ...
        -- 플레이어의 LowerTorso 부분을 가져옵니다.
        LowerTorso = player.Character.LowerTorso
        -- LowerTorso를 게임 월드의 history_tp_part 위치로 순간이동 시킵니다.
...
-- 무한 루프를 돌면서 디바운스를 1초마다 false로 설정하여 재활성화합니다.
while true do
    wait()  -- 잠시 대기
    debounce = false  -- 디바운스를 false로 설정
    wait(1)  -- 1초 대기
end
```
